...
export default function DashboardPage() {
  ...
  function providerDisplayName(provider:Provider){const first=(provider.name||"").trim();const last=(provider.last_name||"").trim();if(provider.business_name?.trim())return provider.business_name.trim();if(provider.name_display==="first")return first||"Not added";if(provider.name_display==="initial"){const initial=last?`${last[0].toUpperCase()}.`:"";return [first,initial].filter(Boolean).join(" ")||"Not added";}return [first,last].filter(Boolean).join(" ")||"Not added";}
  ...
  return <main style={{minHeight:"100vh",background:"#f8f8fb",padding:"40px 20px",fontFamily:"Arial,sans-serif"}}><div style={{maxWidth:960,margin:"0 auto",background:"white",borderRadius:24,padding:"36px",boxShadow:"0 10px 35px rgba(0,0,0,.08)"}}>
    ...
    {provider&&<section style={{marginBottom:34}}><h2 style={{color:"#182033"}}>My Service Listing</h2><p style={{color:"#667085"}}>Manage the service listing connected to this account.</p><div style={{display:"grid",gap:16,gridTemplateColumns:"repeat(auto-fit, minmax(220px, 1fr))"}}><div style={card}><strong>Listing name</strong><div style={muted}>{providerDisplayName(provider)}</div></div><div style={card}><strong>Location</strong><div style={muted}>{[provider.city,provider.state].filter(Boolean).join(", ")||"Not added"}</div></div><div style={card}><strong>Email</strong><div style={muted}>{provider.email||"Not added"}</div></div><div style={card}><strong>Phone</strong><div style={muted}>{provider.phone||"Not added"}</div></div></div><div style={{display:"flex",gap:10,flexWrap:"wrap",marginTop:16}}><a href="/dashboard/edit" style={secondary}>Edit Listing</a><a href={`/provider/${providerSlug(provider)}`} style={secondary}>View Listing</a></div></section>}
    ...
    <a href="/" style={{display:"flex",justifyContent:"center",alignItems:"center",minHeight:48,marginTop:28,background:"#f5f3ff",color:"#4f46e5",padding:"11px 16px",borderRadius:10,textDecoration:"none",fontWeight:700,border:"1px solid #ddd6fe",textAlign:"center",boxSizing:"border-box"}}>← Back to YouListify</a><button onClick={handleSignOut} style={{display:"block",marginTop:18,border:"1px solid #ddd",background:"white",padding:"12px 18px",borderRadius:10,cursor:"pointer",fontWeight:600}}>Sign Out</button>
  </div></main>;
}
