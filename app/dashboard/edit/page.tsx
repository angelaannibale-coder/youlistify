...
<div style={{marginTop:"24px",marginBottom:"24px"}}><div style={{fontWeight:"600",marginBottom:"10px"}}>Pricing</div><label style={{display:"block",marginBottom:"10px"}}><input type="checkbox" checked={form.pricing_methods.includes("hourly")} onChange={(e)=>setForm({...form,pricing_methods:e.target.checked?[...form.pricing_methods,"hourly"]:form.pricing_methods.filter((p)=>p!=="hourly")})}/> Hourly rate</label>{form.pricing_methods.includes("hourly")&&<input type="number" placeholder="Hourly rate $" value={form.starting_price} onChange={(e)=>setForm({...form,starting_price:e.target.value})} style={{width:"100%",padding:"12px",marginBottom:"12px"}}/>}<label style={{display:"block",marginBottom:"10px"}}><input type="checkbox" checked={form.pricing_methods.includes("flat")} onChange={(e)=>setForm({...form,pricing_methods:e.target.checked?[...form.pricing_methods,"flat"]:form.pricing_methods.filter((p)=>p!=="flat")})}/> Flat rate</label>{form.pricing_methods.includes("flat")&&<input type="number" placeholder="Flat rate $" value={form.flat_price} onChange={(e)=>setForm({...form,flat_price:e.target.value})} style={{width:"100%",padding:"12px",marginBottom:"12px"}}/>}<label style={{display:"block",marginBottom:"10px"}}><input type="checkbox" checked={form.pricing_methods.includes("contact")} onChange={(e)=>setForm({...form,pricing_methods:e.target.checked?[...form.pricing_methods,"contact"]:form.pricing_methods.filter((p)=>p!=="contact")})}/> Contact for pricing</label></div>
