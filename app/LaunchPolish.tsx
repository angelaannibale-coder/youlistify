...
export default function LaunchPolish() {
    ...
    (async () => {
      ...
      if (data) {
        ...
        const { data: files } = await supabase.storage
          ...
          .list(String(data.id), { search: "profile-photo" });

        if (files?.some((file) => file.name === "profile-photo")) {
          const { data: publicData } = supabase.storage
            .from("provider-photos")
            .getPublicUrl(`${data.id}/profile-photo`);
  ...
  async function saveMode() {
    if (!providerId) {
      setModeMessage("We could not find your listing.");
      return;
    }
  ...
  async function uploadProfilePhoto(file: File | undefined) {
    ...
    const { error } = await supabase.storage
      .from("provider-photos")
      .upload(`${providerId}/profile-photo`, file, {
        upsert: true,
    ...
    const { data } = supabase.storage
      .from("provider-photos")
      .getPublicUrl(`${providerId}/profile-photo`);
    ...
    <div style={{ margin: "10px 0 24px", padding: "14px 16px", background: "#f8f8fb", border: "1px solid #e7e9f0", borderRadius: "14px" }}>
      ...
      <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
        <select
          ...
          style={{ flex: "1 1 260px", padding: "10px 12px", border: "1px solid #ddd", borderRadius: "10px", fontSize: "15px", background: "white" }}
        >
        ...
        <button
          ...
          onClick={saveMode}
          disabled={savingMode}
          style={{ padding: "10px 18px", border: 0, borderRadius: "10px", background: "#4f46e5", color: "white", fontWeight: 700, cursor: "pointer" }}
        >
  ...
  const profilePhotoBlock = (
    <div style={{ margin: "18px 0 24px", paddingTop: "18px", borderTop: "1px solid #eceef2" }}>
      ...
      <div style={{ fontSize: "14px", color: "#667085", marginBottom: "12px" }}>
        Add one if you want. If not, YouListify will show your initials.
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: "14px", flexWrap: "wrap" }}>
        {profilePhotoUrl ? (
          <img
            src={profilePhotoUrl}
            alt="Profile preview"
            style={{ width: "72px", height: "72px", objectFit: "cover", borderRadius: "18px", border: "1px solid #e5e7eb" }}
          />
        ) : (
          <div style={{ width: "72px", height: "72px", borderRadius: "18px", display: "grid", placeItems: "center", background: colorFor(String(providerId || "YouListify")), color: "white", fontWeight: 800, fontSize: "22px" }}>
            Y
          </div>
        )}
        <div>
      ...
      {profileMessage && (
        <div style={{ marginTop: "9px", fontSize: "13px", color: profileMessage.includes("saved") ? "#15803d" : "#b91c1c" }}>
          {profileMessage}
        </div>
      )}
    </div>
    ...
    <>
      ...
      {contactOpen && (
        <div
          ...
          style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(12,16,35,.56)", display: "grid", placeItems: "center", padding: "20px" }}
        >
          <form
            onSubmit={sendMessage}
            onClick={(e) => e.stopPropagation()}
            style={{ width: "min(100%,560px)", background: "white", borderRadius: "24px", padding: "30px", boxShadow: "0 35px 100px rgba(0,0,0,.25)" }}
          >
