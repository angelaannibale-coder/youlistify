...
{contactOpen && selected && <div className="modal-backdrop" onClick={()=>setContactOpen(false)}><div className="modal provider-contact-modal" onClick={(e)=>e.stopPropagation()}><button className="close" onClick={()=>setContactOpen(false)}>×</button>{contactStatus ? <><h2 style={{color:"#15803d",textAlign:"center"}}>Message sent!</h2><p style={{textAlign:"center"}}>Your message was sent through YouListify.</p><button className="call-action" type="button" onClick={()=>setContactOpen(false)}>Done</button></> : <><h2>Contact {selected.name}</h2><p>Send a private message through YouListify.</p><p style={{color:"#667085",fontSize:"14px",lineHeight:1.55}}>YouListify helps people connect. Please use your own judgment before hiring, meeting, paying, or sharing personal information.</p><input type="text" placeholder="Your name" value={contactName} onChange={(e)=>setContactName(e.target.value)}/><input type="email" placeholder="Your email" value={contactEmail} onChange={(e)=>setContactEmail(e.target.value)}/><textarea placeholder="Your message" value={contactMessage} onChange={(e)=>setContactMessage(e.target.value)}></textarea><button className="call-action" type="button" onClick={sendProviderMessage}>Send message</button></>}</div></div>}
</main>
}
