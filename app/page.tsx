...
const serviceSuggestions = allServices.filter((item: any) => {
  ...
  const rankService = (name: string) =>
    name === serviceSuggestionQuery ? 0 :
    name.startsWith(serviceSuggestionQuery) ? 1 :
    name.split(/\s+/).some((word: string) => word.startsWith(serviceSuggestionQuery)) ? 2 :
    3;
...
<defs><clipPath id="header-smile-visible"><rect x="0" y="38" width="520" height="74"/></clipPath></defs>
<path className="header-smile" d="M140 23Q275 119 410 23" clipPath="url(#header-smile-visible)"/>
...
{showServiceSuggestions && service.trim() && serviceSuggestions.length>0 && (<div style={{position:"absolute",top:"100%",left:0,right:0,zIndex:20,background:"white",border:"1px solid #e6e9f0",borderRadius:"14px",marginTop:"6px",maxHeight:"360px",overflowY:"auto",boxShadow:"0 12px 30px rgba(0,0,0,.12)"}}>{serviceSuggestions.map((item:any)=><button key={item.id} type="button" onPointerDown={(e)=>{e.preventDefault();setService(wantsMobileService?`mobile ${item.name}`:item.name);setShowServiceSuggestions(false);}} onClick={()=>{setService(wantsMobileService?`mobile ${item.name}`:item.name);setShowServiceSuggestions(false);}} style={{display:"block",width:"100%",textAlign:"left",padding:"11px 14px",border:0,background:"white",cursor:"pointer"}}>{wantsMobileService?`Mobile ${item.name}`:item.name}</button>)}</div>)}</label>
...
<footer id="site-footer" className="site-footer"><div className="footer-brand"><a className="brand" href="#top"><span className="footer-logo-symbol" aria-hidden="true"><svg viewBox="0 0 64 64"><path className="footer-sun-rays" d="M32 7v7M17 13l5 6M47 13l-5 6M9 27l8 2M55 27l-8 2"/><path className="footer-sun" d="M22 30a10 10 0 0 1 20 0Z"/><path className="footer-horizon" d="M9 35Q32 26 55 35"/><path className="footer-path" d="M32 35Q42 38 34 41Q23 45 32 48Q41 51 30 56"/></svg></span><span className="brand-word"><span>You</span><span className="capital-l">L</span><span>istify</span></span></a><p>Find the right person. Call them in minutes. Get the job done.</p></div><nav className="footer-links" aria-label="Explore YouListify"><strong>Explore</strong><a href="#search-services" onClick={(e)=>{e.preventDefault();scrollToHomeSection("search-services");}}>Search Services</a><a href="#categories" onClick={(e)=>{e.preventDefault();scrollToHomeSection("categories");}}>Browse Services</a><a href="/list-service">List My Services</a><a href="/work">Jobs / Gigs / Tasks</a></nav><nav className="footer-links" aria-label="YouListify information"><strong>Information</strong><a href="/safety">Safety &amp; Resources</a><a href="/privacy">Privacy Policy</a><a href="/terms">Terms of Use</a><a href="/support">Contact &amp; Support</a></nav><small>© {new Date().getFullYear()} YouListify. All rights reserved.</small></footer>
...
{selected && <div className="modal-backdrop" onClick={()=>setSelected(null)}><div className="modal" onClick={e=>e.stopPropagation()}><button className="close" onClick={()=>setSelected(null)}>×</button><div className="listing-preview-label">LISTING PREVIEW</div><div className="modal-head">{selected.profile_photo?<img className="avatar large" src={selected.profile_photo} alt={`${selected.name} profile`}/>:<div className="avatar large">{selected.initials}</div>}<div>{selected.available_now && <div className="availability"><span className="green-dot"/> Available now</div>}<h2>{selected.name}</h2><p>{selected.category} · {selected.city}</p></div></div><div className="chips">{[...new Set(selected.specialties)].map((s:string)=><span key={s}>{s}</span>)}</div><p className="modal-copy">{selected.response || "No description provided yet."}</p><div style={{display:"flex",flexWrap:"wrap",gap:"10px",justifyContent:"center"}}>{selected.contact_call && selected.phone && <a className="call-action" href={`tel:${selected.phone.replace(/\D/g,"")}`}>☎ Call {selected.phone}</a>}{selected.contact_text && selected.phone && <a className="call-action" href={`sms:${selected.phone.replace(/\D/g,"")}`}>💬 Text</a>}{selected.contact_email && selected.email && <a className="call-action" href={`mailto:${selected.email}`}>✉ Email</a>}{selected.contact_youlistify && <button className="call-action" type="button" onClick={()=>{setContactStatus("");setContactOpen(true);}}>✉ Contact through YouListify</button>}</div><a href={`/provider/${selected.slug}`} className="call-action listing-preview-full" style={{display:"block",width:"100%",textAlign:"center",marginTop:"14px",fontSize:"17px",fontWeight:"700",textDecoration:"none"}}>View Full Listing</a></div></div>}

{contactOpen && selected && <div className="modal-backdrop" onClick={()=>setContactOpen(false)}><div className="modal provider-contact-modal" onClick={(e)=>e.stopPropagation()}><button className="close" onClick={()=>setContactOpen(false)}>×</button>{contactStatus ? <><h2 style={{color:"#15803d",textAlign:"center"}}>Message sent!</h2><p style={{textAlign:"center"}}>Your message was sent through YouListify.</p><button className="call-action" type="button" onClick={()=>setContactOpen(false)}>Done</button></> : <><h2>Contact {selected.name}</h2><p>Send a private message through YouListify.</p><input type="text" placeholder="Your name" value={contactName} onChange={(e)=>setContactName(e.target.value)}/><input type="email" placeholder="Your email" value={contactEmail} onChange={(e)=>setContactEmail(e.target.value)}/><textarea placeholder="Your message" value={contactMessage} onChange={(e)=>setContactMessage(e.target.value)}></textarea><button className="call-action" type="button" onClick={sendProviderMessage}>Send message</button></>}</div></div>}
</main>
}
