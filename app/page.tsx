...
<label style={{display:"flex",alignItems:"center",gap:"8px",fontWeight:"600",whiteSpace:"nowrap"}}><input type="checkbox" checked={remoteSearch} onChange={(e)=>{const checked=e.target.checked;setRemoteSearch(checked);if(checked){setLocation("Remote / Online / Phone");setShowLocationSuggestions(false);}else if(/^remote(?:\\s|\\/|$)/i.test(location.trim())){setLocation("");}}} />Remote / Online / Phone</label>
