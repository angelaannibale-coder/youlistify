...
export default function EditWorkPostPage(){
  ...
  useEffect(()=>{function close(e:MouseEvent){if(categoryBox.current&&!categoryBox.current.contains(e.target as Node))setShowCategorySuggestions(false);}document.addEventListener("mousedown",close);return()=>document.removeEventListener("mousedown",close);},[]);

  const set=(key:string,value:any)=>setForm((v:any)=>({...v,[key]:value}));
  const categorySuggestions=categories.filter(c=>c.toLowerCase().includes(String(form?.category||"").toLowerCase())).slice(0,10);
