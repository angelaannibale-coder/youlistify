...
export default function EditWorkPostPage(){
  ...
  <div style={{display:"grid",gap:16}}><input required style={input} placeholder="Title" value={form.title||""} onChange={e=>set("title",e.target.value)}/><div ref={categoryBox} style={{position:"relative"}}><input style={input} placeholder="Category" value={form.category||""} onFocus={()=>setShowCategorySuggestions(true)} onChange={e=>{set("category",e.target.value);setShowCategorySuggestions(true);}} autoComplete="off"/>{showCategorySuggestions&&categorySuggestions.length>0&&<div style={{position:"absolute",zIndex:20,top:"calc(100% + 6px)",left:0,right:0,background:"white",border:"1px solid #e2e4ea",borderRadius:12,boxShadow:"0 10px 28px rgba(0,0,0,.12)",maxHeight:260,overflowY:"auto"}}>{categorySuggestions.map((cat,i)=><button key={cat} type="button" onClick={()=>{set("category",cat);setShowCategorySuggestions(false);}} style={{display:"block",width:"100%",textAlign:"left",padding:"12px 14px",background:"white",border:0,borderBottom:i<categorySuggestions.length-1?"1px solid #f0f1f4":"none",cursor:"pointer",fontSize:15,color:"#171b36"}}>{cat}</button>)}</div>}</div><textarea required style={{...input,minHeight:150,resize:"vertical"}} placeholder="Description" value={form.description||""} onChange={e=>set("description",e.target.value)}/></div>
  <h2 style={{marginTop:30}}>Where?</h2><label style={check}><input type="checkbox" checked={!!form.remote} onChange={e=>set("remote",e.target.checked)}/> Remote / can be done from anywhere</label><div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:10}}><div ref={cityBox} style={{position:"relative"}}><input style={input} placeholder="City" value={form.city||""} onFocus={()=>citySuggestions.length&&setShowCitySuggestions(true)} onChange={e=>{set("city",e.target.value);setShowCitySuggestions(true);}} autoComplete="off"/>{showCitySuggestions&&citySuggestions.length>0&&<div style={{position:"absolute",zIndex:20,top:"calc(100% + 6px)",left:0,right:0,background:"white",border:"1px solid #e2e4ea",borderRadius:12,boxShadow:"0 10px 28px rgba(0,0,0,.12)",maxHeight:240,overflowY:"auto"}}>{citySuggestions.map((s,i)=><button key={`${s.city}-${s.state}-${i}`} type="button" onClick={()=>{setForm((v:any)=>({...v,city:s.city,state:s.state}));setShowCitySuggestions(false);}} style={{display:"block",width:"100%",textAlign:"left",padding:"12px 14px",background:"white",border:0,borderBottom:i<citySuggestions.length-1?"1px solid #f0f1f4":"none",cursor:"pointer",fontSize:15,color:"#171b36"}}>{s.city}, {s.state}</button>)}</div>}</div><select style={input} value={form.state||""} onChange={e=>set("state",e.target.value)}><option value="">State</option>{states.map(s=><option key={s}>{s}</option>)}</select><input style={input} placeholder="ZIP" inputMode="numeric" value={form.zip_code||""} onChange={e=>set("zip_code",e.target.value)}/></div>
