...
export default function PostWorkPage() {
  ...
  const locationChoices=[["local","Local only","Must be near this city"],["remote","Remote only","Can be done from anywhere"],["both","Local and remote","Tied to this city, but remote is okay"]] as const;
  ...
  <h2 style={{marginTop:30,marginBottom:10}}>Where?</h2><p style={{color:"#667085",lineHeight:1.5,margin:"0 0 12px"}}>Choose how the work can be done. City, state, and ZIP stay as the post's base location even when remote is allowed.</p><label style={{fontWeight:800,display:"block",marginBottom:8}}>Location type</label><div style={{display:"grid",gap:10,marginBottom:14}}>{locationChoices.map(([mode,label,detail])=><button key={mode} type="button" onClick={()=>setLocationChoice(mode)} aria-pressed={locationMode===mode} style={{padding:"13px 14px",borderRadius:12,border:locationMode===mode?"2px solid #5b4df5":"1px solid #d9dce7",background:locationMode===mode?"#f1efff":"white",textAlign:"left",cursor:"pointer"}}><span style={{display:"block",fontWeight:900,color:"#171b36",fontSize:16}}>{label}</span><span style={{display:"block",color:"#667085",fontSize:13,lineHeight:1.35,marginTop:3}}>{detail}</span></button>)}</div><div style={{display:"grid",gridTemplateColumns:"minmax(0,2fr) minmax(88px,1fr) minmax(100px,1fr)",gap:10}}><div ref={cityBox} style={{position:"relative"}}><input style={input} placeholder="City" value={form.city} onFocus={()=>citySuggestions.length&&setShowCitySuggestions(true)} onChange={e=>{set("city",e.target.value);setShowCitySuggestions(true);}} autoComplete="off"/>{showCitySuggestions&&citySuggestions.length>0&&<div style={{position:"absolute",zIndex:20,top:"calc(100% + 6px)",left:0,right:0,background:"white",border:"1px solid #e2e4ea",borderRadius:12,boxShadow:"0 10px 28px rgba(0,0,0,.12)",maxHeight:240,overflowY:"auto"}}>{citySuggestions.map((s,i)=><button key={`${s.city}-${s.state}-${i}`} type="button" onClick={()=>{setForm(v=>({...v,city:s.city,state:s.state}));setShowCitySuggestions(false);}} style={{display:"block",width:"100%",textAlign:"left",padding:"12px 14px",background:"white",border:0,borderBottom:i<citySuggestions.length-1?"1px solid #f0f1f4":"none",cursor:"pointer",fontSize:15,color:"#171b36"}}>{s.city}, {s.state}</button>)}</div>}</div><select style={input} value={form.state} onChange={e=>set("state",e.target.value)}><option value="">State</option>{states.map(s=><option key={s}>{s}</option>)}</select><input style={input} placeholder="ZIP" inputMode="numeric" value={form.zip_code} onChange={e=>set("zip_code",e.target.value)}/></div>
