...
export default function PostWorkPage() {
  ...
  useEffect(()=>{
    ...
    supabase.auth.getSession().then(({data})=>{
      ...
      setUserId(user?.id||"");
      if(saved?.location_mode)setLocationMode(saved.location_mode);
      else if(saved?.remote)setLocationMode("both");
  ...
  const set=(key:string,value:any)=>{if(["city","state","zip_code"].includes(key))setLocationError("");setForm(v=>({...v,[key]:value}));};
  ...
  async function submit(e:FormEvent){
    ...
    if(missingLocation.length>0){setLocationError(`Please fill in the ${missingLocation.join(", ")} above before posting. Even remote posts need a base city, state, and ZIP code so people know where the post is connected to.`);return;}

    if(!userId){
      localStorage.setItem(draftKey,JSON.stringify({...form,location_mode:locationMode}));
    ...
    localStorage.setItem(draftKey,JSON.stringify({...form,location_mode:locationMode}));
    setSaving(true);
    const {data,error}=await supabase.from("work_posts").insert({...form,location_mode:locationMode,remote:locationMode!=="local",user_id:userId,pay_amount:form.pay_amount?Number(form.pay_amount):null,contact_phone:form.contact_phone.trim()||null,contact_email_address:form.contact_email_address.trim()||null,application_url:form.application_url.trim()||null}).select("id").single();
    setSaving(false);
    if(error){
  ...
  <h2 style={{marginTop:30,marginBottom:10}}>Where?</h2><p style={{color:"#667085",lineHeight:1.5,margin:"0 0 10px"}}>Check what applies, then enter the post's base location.</p><div style={{display:"grid",gap:4,marginBottom:8}}><label style={{...check,padding:"6px 0",cursor:"pointer",fontWeight:800}}><input type="checkbox" checked={locationMode!=="local"} onChange={e=>setLocationChoice(e.target.checked?(locationMode==="local"?"remote":"both"):(locationMode==="both"?"local":"local"))}/><span>Remote / can be done from anywhere</span></label><label style={{...check,padding:"6px 0",cursor:"pointer",fontWeight:800}}><input type="checkbox" checked={locationMode!=="remote"} onChange={e=>setLocationChoice(e.target.checked?(locationMode==="remote"?"both":"local"):(locationMode==="both"?"remote":"remote"))}/><span>Local / tied to this city</span></label></div><p style={{color:"#667085",fontSize:14,lineHeight:1.45,margin:"0 0 14px"}}>You can check both if the post is local and remote.</p><div style={{display:"grid",gridTemplateColumns:"minmax(0,2fr) minmax(88px,1fr) minmax(100px,1fr)",gap:10}}><div ref={cityBox} style={{position:"relative"}}><input style={input} placeholder="City" value={form.city} onFocus={()=>citySuggestions.length&&setShowCitySuggestions(true)} onChange={e=>{set("city",e.target.value);setShowCitySuggestions(true);}} autoComplete="off"/>{showCitySuggestions&&citySuggestions.length>0&&<div style={{position:"absolute",zIndex:20,top:"calc(100% + 6px)",left:0,right:0,background:"white",border:"1px solid #e2e4ea",borderRadius:12,boxShadow:"0 10px 28px rgba(0,0,0,.12)",maxHeight:240,overflowY:"auto"}}>{citySuggestions.map((s,i)=><button key={`${s.city}-${s.state}-${i}`} type="button" onClick={()=>{setLocationError("");setForm(v=>({...v,city:s.city,state:s.state}));setShowCitySuggestions(false);}} style={{display:"block",width:"100%",textAlign:"left",padding:"12px 14px",background:"white",border:0,borderBottom:i<citySuggestions.length-1?"1px solid #f0f1f4":"none",cursor:"pointer",fontSize:15,color:"#171b36"}}>{s.city}, {s.state}</button>)}</div>}</div><select style={input} value={form.state} onChange={e=>set("state",e.target.value)}><option value="">State</option>{states.map(s=><option key={s}>{s}</option>)}</select><input style={input} placeholder="ZIP" inputMode="numeric" value={form.zip_code} onChange={e=>set("zip_code",e.target.value)}/></div>{locationError&&<div role="alert" style={{marginTop:10,color:"#9f1d1d",background:"#fff8f7",border:"1px solid #f6d7d2",borderRadius:10,padding:"10px 12px",fontWeight:700,lineHeight:1.4}}>{locationError}</div>}
